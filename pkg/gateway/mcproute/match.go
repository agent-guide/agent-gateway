...
func (m *Manager) Match(ctx context.Context, r *http.Request) (MCPRoute, bool, error) {
	routes, err := m.List(ctx, RouteListOptions{})
	if err != nil {
		return MCPRoute{}, false, err
	}
	...
	for _, route := range routes {
		if route.Disabled || !routeMatchesRequest(route.Match, r) {
			continue
		}
