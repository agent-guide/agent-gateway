...
import (
	...
	runtimeollama "github.com/agent-guide/caddy-agent-gateway/pkg/llm/provider/ollama"
...
func (p *Provider) Provision(_ caddy.Context) error {
	if err := caddyprovider.ValidateProviderType(&p.ProviderConfig, "ollama"); err != nil {
		return err
	}
	built, err := runtimeollama.New(p.ProviderConfig)
	if err != nil {
		return err
	}
	p.Runtime = built
