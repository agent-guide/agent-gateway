...
import (
	...
	runtimezhipu "github.com/agent-guide/caddy-agent-gateway/pkg/llm/provider/zhipu"
...
func (p *Provider) Provision(_ caddy.Context) error {
	if err := caddyprovider.ValidateProviderType(&p.ProviderConfig, "zhipu"); err != nil {
		return err
	}
	built, err := runtimezhipu.New(p.ProviderConfig)
	if err != nil {
		return err
	}
	p.Runtime = built
