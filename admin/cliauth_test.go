...
func TestCLIAuthResolvesAuthenticatorAndRegistersCredential(t *testing.T) {
	...
	handler := NewHandler(newTestAgentGateway(nil, cliauthMgr, nil), nil, "admin", string(passwordHash), nil)
...
func TestCLIAuthReturnsNotFoundForUnknownCliname(t *testing.T) {
	...
	handler := NewHandler(newTestAgentGateway(nil, manager.NewManager(nil, nil, nil), nil), nil, "admin", string(passwordHash), nil)
	token := loginForTest(t, handler, "admin", "secret-pass")
