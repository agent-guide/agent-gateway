...
func (h *Handler) handleListCaddyServers(w http.ResponseWriter, r *http.Request) {
	if h.caddyManager == nil {
		_ = utils.WriteError(w, http.StatusServiceUnavailable, "caddy manager not configured")
		return
	}
	...
	_ = utils.WriteJSON(w, http.StatusOK, map[string]any{"items": servers})
}
...
func (h *Handler) handleGetCaddyServer(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, caddymgr.ErrNotFound) {
			_ = utils.WriteError(w, http.StatusNotFound, "server not found")
			return
		}
		_ = utils.WriteError(w, http.StatusInternalServerError, err.Error())
		return
	}
	_ = utils.WriteJSON(w, http.StatusOK, srv)
}
...
func (h *Handler) handleCreateCaddyServer(w http.ResponseWriter, r *http.Request) {
	...
	_ = utils.WriteJSON(w, http.StatusCreated, srv)
}
...
func (h *Handler) handleUpdateCaddyServer(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.caddyManager.UpdateServer(r.Context(), &req); err != nil {
		if errors.Is(err, caddymgr.ErrReadOnly) {
			_ = utils.WriteError(w, http.StatusForbidden, err.Error())
			return
		}
		if errors.Is(err, caddymgr.ErrNotFound) {
			_ = utils.WriteError(w, http.StatusNotFound, "server not found")
...
func (h *Handler) handleListCaddyRoutes(w http.ResponseWriter, r *http.Request) {
	...
	_ = utils.WriteJSON(w, http.StatusOK, map[string]any{"items": routes})
}
...
func (h *Handler) handleAddCaddyRoute(w http.ResponseWriter, r *http.Request) {
	...
	_ = utils.WriteJSON(w, http.StatusCreated, map[string]string{"id": req.ID})
}
...
func (h *Handler) handleUpdateCaddyRoute(w http.ResponseWriter, r *http.Request) {
	...
	for _, route := range routes {
		if route.ID == routeID {
			_ = utils.WriteJSON(w, http.StatusOK, route)
			return
		}
	}
	_ = utils.WriteJSON(w, http.StatusOK, map[string]string{"id": routeID})
}
...
func (h *Handler) handleDeleteCaddyRoute(w http.ResponseWriter, r *http.Request) {
	...
	_ = utils.WriteJSON(w, http.StatusOK, map[string]string{"status": "deleted", "id": routeID})
}
