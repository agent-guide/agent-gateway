...
This document extends the MCP gateway architecture described in `architecture/mcp-architecture.md` and fills in the currently unimplemented metrics area referenced by `architecture/architecture-overview.md`. MCP tool policy is a separate concern described in `mcp-tool-policy.md`.
...
- policy attribution: presented tool name, executed tool name, execution mode (see `mcp-tool-policy.md`)
...
presented_tool_name string     client-visible tool name after alias/policy application
                               (see mcp-tool-policy.md for how this is set)
...
The `presented_tool_name`, `executed_tool_name`, and `execution_mode` fields are populated by the tool policy layer in `pkg/mcp/service/`. See `mcp-tool-policy.md` §6.7.7 for audit attribution details.
...
- pass the resolved route into policy-aware tool listing and tool execution helpers (see `mcp-tool-policy.md` §9)
...
- once implementation lands, `architecture/architecture-overview.md` should be updated so metrics no longer appear as future work
