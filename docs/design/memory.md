...
- `observability.md` for trace propagation and audit correlation
- `architecture/mcp-architecture.md` for shared dispatcher and routecore boundaries
- `mcp-tool-policy.md` for the route-level policy model pattern
...
This document assumes option 2 because it preserves the current protocol-aware handler architecture described in `architecture/mcp-architecture.md` while creating a reusable execution seam.
...
Trace propagation follows `observability.md`.
...
Memory events are part of the unified observability model from `observability.md`, not a separate disconnected telemetry system.
