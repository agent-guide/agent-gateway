...
- 如果一次性移动所有目录，测试和文档会出现大量无关 diff；但对 `admin` 和 `dispatcher` 这类薄适配层，不值得保留 compatibility shell，应直接更新调用方和测试。
...
- 从 Caddy 模块中剥离出的 adapter 应优先迁入 `caddy/...`，不要长期留在旧 runtime 路径。`admin` 和 `dispatcher` 完成迁移后不保留 alias、shim 或 test-only compatibility wrapper。
...
- 不保留 `admin` / `dispatcher` 的 compatibility shell；测试应优先使用包内测试，删除仅为跨包测试暴露的解析辅助导出。
