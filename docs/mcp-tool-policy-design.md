...
This document extends the MCP gateway architecture described in `mcp-gateway-architecture.md`. Observability for tool policy executions is defined in `observability-design.md`; the `presented_tool_name`, `executed_tool_name`, and `execution_mode` audit fields on `MCPUsageEvent` capture policy attribution.
...
```json
{
  "content": [
    {
      "type": "text",
      "text": "{\"action\":\"changed_files\",\"count\":2,\"files\":[\"pkg/dispatcher/mcp_handler.go\",\"docs/mcp-tool-policy-design.md\"]}"
    }
  ]
}
```
...
Synthetic replacement extends the audit model defined in `observability-design.md`. The `MCPUsageEvent` already carries `presented_tool_name`, `executed_tool_name`, `execution_mode`, and `policy_action` fields from phase 1. The tool policy layer is responsible for populating these fields when it resolves the tool call:
...
5. Populate `presented_tool_name`, `executed_tool_name`, and `execution_mode` on the `InteractionSpan` for synthetic execution paths (see `observability-design.md` §7.1)
