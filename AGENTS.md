...
- codex-acp (verified live against v0.16.0) does NOT push `session_info_update`, so the `session_info` SSE event never fires for codex and its cached session info stays empty; the session title for a codex session is only available through `session/list` (`GET /admin/acp/services/{id}/sessions` → `sessions[].title`, parsed by `parseListSessionsResponse` into `SessionInfo.Title`), which codex auto-derives from the first user message. The `session_info` snapshot/SSE path remains for agents that do emit `session_info_update`.
